...
class Audio2landmark_model:
    ...
    def __init__(self, opt_parser, jpg_shape=None):
        ...
        self.std_face_id = torch.tensor(
            self.std_face_id, requires_grad=False, dtype=torch.float
        ).to(device)

        self.eval_data = Audio2landmark_Dataset(
        ...
        print(
            "G: Running on {}, total num params = {:.2f}M".format(
        ...
        ckpt = torch.load(
            opt_parser.load_a2l_G_name, map_location=torch.device("cpu")
        )  # delete map_location argument if gpu
        pretrained_dict = {
            k: v for k, v in ckpt["G"].items() if k.split(".")[0] not in ["comb_mlp"]
        }
        model_dict.update(pretrained_dict)
        self.G.load_state_dict(model_dict)

        print(
            "======== LOAD PRETRAINED FACE ID MODEL {} =========".format(
        ...
        ckpt = torch.load(
            opt_parser.load_a2l_C_name, map_location=torch.device("cpu")
        )  # here too
    ...
    def __train_face_and_pos__(
        self, fls, aus, embs, face_id, smooth_win=31, close_mouth_ratio=0.99
    ):
        ...
        z = torch.tensor(
            torch.zeros(aus.shape[0], 128), requires_grad=False, dtype=torch.float
        ).to(device)
        ...
        fl_dis_pred[:, 49:54] = mean_out * close_mouth_ratio + fl_dis_pred[:, 49:54] * (
            1 - close_mouth_ratio
        )
        fl_dis_pred[:, index1] = mean_out * close_mouth_ratio + fl_dis_pred[
            :, index1
        ] * (1 - close_mouth_ratio)
        mean_in = 0.5 * (fl_dis_pred[:, 61:64] + fl_dis_pred[:, index2])
        fl_dis_pred[:, 61:64] = mean_in * close_mouth_ratio + fl_dis_pred[:, 61:64] * (
            1 - close_mouth_ratio
        )
        fl_dis_pred[:, index2] = mean_in * close_mouth_ratio + fl_dis_pred[
            :, index2
        ] * (1 - close_mouth_ratio)
    ...
    def __calib_baseline_pred_fls_old_(self, baseline_pred_fls, residual_face_id, aus):
        ...
        residual_face_id -= mean_face_id.view(1, 204) * 1.0
    ...
    def __train_pass__(
        self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False
    ):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in keys:  # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                inputs_fl, inputs_au, inputs_emb = (
                    inputs_fl.to(device),
                    inputs_au.to(device),
                    inputs_emb.to(device),
                )
                ...
                for j in range(0, inputs_fl.shape[0], seg_bs):
                    ...
                    fl_dis_pred_pos, input_face_id = self.__train_face_and_pos__(
                        inputs_fl_segments,
                        inputs_au_segments,
                        inputs_emb_segments,
                        input_face_id,
                ...
                fake_fls_np[:, 27 * 3 : 28 * 3] = (
                    fake_fls_np[:, 28 * 3 : 29 * 3] * 2
                    - fake_fls_np[:, 29 * 3 : 30 * 3]
                )
                ...
                if centerize_face:
                    ...
                    fake_fls_np = (
                        fake_fls_np
                        - np.mean(fake_fls_np, axis=1, keepdims=True)
                        + std_m
                    )
                    fake_fls_np = fake_fls_np.reshape((-1, 68 * 3))
                ...
                np.savetxt(
                    os.path.join(self.opt_parser.output_folder, filename),
                    fake_fls_np,
                    fmt="%.6f",
                )
                ...
                if vis_fls:
                    ...
                    Vis(
                        fls=fake_fls_np,
                        filename=video_name.split("\\")[-1].split("/")[-1],
                        fps=62.5,
                        audio_filenam=os.path.join(
                            "examples",
    ...
    def __solve_inverse_lip2__(self, fl_dis_pred_pos_numpy):
        for j in range(fl_dis_pred_pos_numpy.shape[0]):
            ...
            if area_of_mouth < 0:
                ...
                fl_dis_pred_pos_numpy[j, 63 * 3 : 64 * 3] = fl_dis_pred_pos_numpy[
                    j, 65 * 3 : 66 * 3
                ]
                ...
                fl_dis_pred_pos_numpy[j, 62 * 3 : 63 * 3] = fl_dis_pred_pos_numpy[
                    j, 66 * 3 : 67 * 3
                ]
                ...
                fl_dis_pred_pos_numpy[j, 61 * 3 : 62 * 3] = fl_dis_pred_pos_numpy[
                    j, 67 * 3 : 68 * 3
                ]
                p = max([j - 1, 0])
                fl_dis_pred_pos_numpy[j, 55 * 3 + 1 : 59 * 3 + 1 : 3] = (
                    fl_dis_pred_pos_numpy[j, 64 * 3 + 1 : 68 * 3 + 1 : 3]
                    + fl_dis_pred_pos_numpy[p, 55 * 3 + 1 : 59 * 3 + 1 : 3]
                    - fl_dis_pred_pos_numpy[p, 64 * 3 + 1 : 68 * 3 + 1 : 3]
                )
                fl_dis_pred_pos_numpy[j, 59 * 3 + 1 : 60 * 3 + 1 : 3] = (
                    fl_dis_pred_pos_numpy[j, 60 * 3 + 1 : 61 * 3 + 1 : 3]
                    + fl_dis_pred_pos_numpy[p, 59 * 3 + 1 : 60 * 3 + 1 : 3]
                    - fl_dis_pred_pos_numpy[p, 60 * 3 + 1 : 61 * 3 + 1 : 3]
                )
                fl_dis_pred_pos_numpy[j, 49 * 3 + 1 : 54 * 3 + 1 : 3] = (
                    fl_dis_pred_pos_numpy[j, 60 * 3 + 1 : 65 * 3 + 1 : 3]
