...
def calculate_evidence(patch_responses, rate=0.25, offset=20):
    ...
    varList = [
        (
            np.abs(
                np.average(
                    (y_coordinate - np.average(y_coordinate, weights=y_weight[i])) ** 2,
                    weights=y_weight[i],
                )
            ),
            np.abs(
                np.average(
                    (x_coordinate - np.average(x_coordinate, weights=x_weight[i])) ** 2,
                    weights=x_weight[i],
                )
            ),
        )
    ...
    prpList = [
        (
            np.sum(patch_responses[i, 0, ...], axis=(-1, -2)),
            np.sum(patch_responses[i, 0, ...], axis=(-1, -2)),
        )
        for i in range(n_points)
    ]
...
def get_patches_around_landmarks(
    heat_maps, menpo_shape, patch_size=(30, 30), image_shape=256
):
    # from ECT: https://github.com/HongwenZhang/ECT-FaceAlignment
    ...
    rspList = [
        rps_zeros[0, y - rOffset : y + lOffset, x - rOffset : x + lOffset, i]
        for i in range(menpo_shape.n_points)
...
def weighted_pdm_transform(input_pdm_model, patches, shape, inirho=20):
    ...
    ini_rho2_inv_prior = np.hstack(
        (np.zeros((4,)), inirho / pdm_model.model.eigenvalues)
    )
...
def w_pdm_correct(init_shape, patches, pdm_model, part_inds=None):
    ...
    points = weighted_pdm_transform(
        input_pdm_model=pdm_model, patches=patches, shape=PointCloud(init_shape)
    )
...
def feature_based_pdm_corr(lms_init, models_dir, train_type="basic", patches=None):
    ...
    for i, part in enumerate(parts):
        ...
        with open(temp_model, "r") as f: #quick fix 1
            mod_f = f.read().replace('\r\n', '\n')

        with open(temp_model, 'w') as file:
            file.write(mod_f)
...
def clm_correct(clm_model_path, image, map, lms_init):
    ...
    fr = fitter.fit_from_shape(
        image=image, initial_shape=PointCloud(lms_init), gt_shape=PointCloud(lms_init)
    )
